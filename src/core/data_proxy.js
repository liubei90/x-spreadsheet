...
export default class DataProxy {
  ...
  copyToSystemClipboard(evt) {
    ...
    try {
      ...
      // // 2 3 4 6
      helper.eachRow(data, (row, ri) => {
        if (ri >= sri && ri <= eri) {
          helper.eachCell(row, (cell, ci) => {
            if (ci >= sci && ci <= eci) {
              const copyCell = {
                ...cell,
              };
              if (cell.style || cell.style === 0) {
                const cellStyle = styles[cell.style];
                if (
                  copyStyleMap[cell.style] ||
                  copyStyleMap[cell.style] === 0
                ) {
                  copyCell.style = copyStyleMap[cell.style];
                } else {
                  copyData.styles.push(cellStyle);
                  copyStyleMap[cell.style] = copyData.styles.length - 1;
                  copyCell.style = copyStyleMap[cell.style];
                }
              }
              if (!copyData.rows[ri]) {
              ...
              copyData.rows[ri].cells[ci] = copyCell;
            }
          });
        }
      });

      copyText = "spreadsheet:" + JSON.stringify(copyData);
      ...
      if (content.slice(0, 12) === "spreadsheet:") {
        try {
          this.changeData(() => {
            ...
            const copyStyles = copyData.styles || [];
            const { rows, merges } = this;
            const dstCellRange = selector.range;
            rows.deleteCells(dstCellRange, "all");
            merges.deleteWithin(dstCellRange);

            const sri = copyData.range.sri;
            const sci = copyData.range.sci;
            const dsri = dstCellRange.sri;
            const dsci = dstCellRange.sci;
            const deri = dstCellRange.eri;
            const deci = dstCellRange.eci;

            helper.eachRow(copyData, (row, ri) => {
              helper.eachCell(row, (cell, ci) => {
                if (typeof cell.style === "number" && copyStyles[cell.style]) {
                  const style = this.addStyle(copyStyles[cell.style]);
                  cell.style = style;
                }
